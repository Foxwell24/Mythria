...
public class RenderWaterBolt<T extends Entity> extends Render<T> {


    public RenderWaterBolt(final RenderManager renderManager) {
        super(renderManager);
    }
    ...
    public void doRender(final T entity, final double x, final double y, final double z, final float entityYaw,
        ...
        for (int i = 0; i < 3; i++) {
            ...
            entity.world.spawnParticle(EnumParticleTypes.DRIP_WATER,
                    entity.posX + vec.x / 4,
                    entity.posY + vec.y / 4,
                    entity.posZ + vec.z / 4,
                    0, 0, 0);
